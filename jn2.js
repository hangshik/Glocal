...
function clickbt(j) {
  ...
  function main() {
    ...
    document.imgp.width=sw/2;
    // document.imgp.width="500";

    let tag = "<div class='container'>";
      for (i = 0; i < 22; i++) {
        list.push(0);
        list_number.push(0);
        tag += "<div class='column' id='"+i+"' style=width:"+sh/20+"px;height:30px;left:"+jn_left[i]*sw/750+"px;top:"+jn_top[i]*sw/750+"px; onclick='clickbt("+i+")'></div>";      }
      tag += "</div>";
      area.innerHTML = tag;
      console.log=tag;
